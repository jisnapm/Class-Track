      ...
      if (enrollStep !== null && enrollStep < 2) {
        ...
        updateState(prev => ({
          ...
          users: prev.users.map(u => 
            u.id === prev.currentUser?.id ? { ...u, enrolledFaces: newFaces, canReenroll: false } : u
          ),
          currentUser: prev.currentUser ? { ...prev.currentUser, enrolledFaces: newFaces, canReenroll: false } : null
        }));
        setEnrollStep(null);
        setCapturedFaces([]);
      }
      ...
      <div className="min-h-screen bg-indigo-900 flex items-center justify-center p-4">
        <div className="max-w-xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-500">
          <div className="p-8 text-center border-b border-gray-100 bg-gray-50/50">
            ...
            <h2 className="text-2xl font-black text-gray-900 tracking-tight">Recalibrate Step {enrollStep + 1}</h2>
            <p className="text-gray-500 mt-1 font-medium">{current.instruction}</p>
          </div>
          <div className="p-8">
            <Camera onCapture={handleCaptureStep} isProcessing={isProcessing} label={`Capture ${current.title}`} />
          </div>
          <div className="p-4 bg-gray-50 text-center">
            <button onClick={() => setEnrollStep(null)} className="text-gray-400 hover:text-red-500 transition-colors text-sm font-bold">Cancel</button>
          </div>
        </div>
      </div>
    );
      ...
      <nav className="lg:w-80 bg-white border-r border-gray-200 p-8 flex flex-col">
        ...
        <div className="flex-1 space-y-4">
           ...
           <div className="space-y-2 pt-4">
              ...
              <button 
                disabled={!state.currentUser?.canReenroll}
                onClick={() => setEnrollStep(0)}
                className={`w-full text-left px-4 py-3 rounded-xl transition-all flex items-center justify-between group ${
                  state.currentUser?.canReenroll 
              ...
              >
                ...
                {!state.currentUser?.canReenroll && <span className="text-[9px] bg-gray-100 px-1.5 py-0.5 rounded-md font-black uppercase text-gray-400">Locked</span>}
              </button>
        ...
        {!state.currentUser?.canReenroll && (
          <div className="mt-8 p-4 bg-amber-50 rounded-2xl border border-amber-100">
            <p className="text-[10px] text-amber-700 font-bold leading-relaxed">
              Biometric updates require Admin permission. Contact IT to request a face reset.
            </p>
          </div>
        )}
      </nav>
               ...
               <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                     {myAttendance.length === 0 ? (
                       <tr><td colSpan={4} className="px-8 py-10 text-center text-gray-400 italic">No biometric records logged yet.</td></tr>
                     ) : (
